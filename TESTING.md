...
Los tests de cuentas, categorías, movimientos y presupuestos contra PostgreSQL requieren una base aislada, migrada y con un nombre terminado en `_test`:
...
Estas pruebas limpian las tablas `budgets`, `transactions`, `categories` y `accounts` (en ese orden, por las claves foráneas); nunca las apuntes a desarrollo o producción.
