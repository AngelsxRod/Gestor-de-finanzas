...
La API expone `GET /api/v1/health`, `GET /api/v1/accounts`, `POST /api/v1/accounts`, `PATCH /api/v1/accounts/:id`, `PATCH /api/v1/accounts/:id/active`, `GET /api/v1/categories`, `POST /api/v1/categories`, `PATCH /api/v1/categories/:id`, `PATCH /api/v1/categories/:id/active`, `GET /api/v1/transactions`, `POST /api/v1/transactions`, `PATCH /api/v1/transactions/:id` y `PATCH /api/v1/transactions/:id/active`. Las peticiones y respuestas públicas usan contratos Zod de `@gestor-finanzas/contracts`; la web las consume mediante el rewrite de Next.js. Los endpoints `/active` son la única forma de "eliminar" en los tres módulos: desactivan o reactivan el registro (ver [ADR-0002](../../docs/adr/0002-soft-delete-simetrico-cuentas-categorias.md) y [ADR-0003](../../docs/adr/0003-desactivacion-reversible-movimientos.md)); no existe borrado físico público.

La aplicación registra un prefijo versionado, `StandardSchemaValidationPipe` y un `DatabaseModule` global. `DatabaseService` expone el cliente tipado de `@gestor-finanzas/models` y lo cierra durante el apagado. `AccountsModule`, `CategoriesModule` y `TransactionsModule` implementan cada uno su controller, servicio y repository. `TransactionsService` deriva la moneda del movimiento de la cuenta seleccionada (nunca la recibe del cliente) y valida contra `AccountsRepository`/`CategoriesRepository` lo que una sola tabla no puede expresar: que la cuenta y, si aplica, la categoría existan y estén activas; que el tipo de categoría coincida con el del movimiento; que la cuenta destino de una transferencia sea distinta de la de origen; y que ambas cuentas de una transferencia compartan moneda. `PATCH /api/v1/transactions/:id` reutiliza exactamente estas reglas, así que editar un movimiento cuya cuenta o categoría fue desactivada después de crearlo falla salvo que la edición también corrija esa referencia. No hay autenticación ni autorización.
