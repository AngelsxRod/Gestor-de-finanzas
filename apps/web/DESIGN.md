...
- `movimientos` no reutiliza `createModalContext<T>()`: como todavía no tiene modo edición, `TransactionModalProvider` (`src/features/transactions/context/transaction-modal-context.tsx`) es un context a medida con solo `isOpen`/`open`/`close`, sin la rama `openEdit` que no tendría uso. Cuando se agregue edición de movimientos, conviene migrarlo al mismo factory que usan cuentas y categorías.
