...
La aplicación es un dashboard financiero: un `AppShell` (Server Component) compone una barra lateral de navegación (`Sidebar`) y el área principal (`MainRegion`), y cada página arma su propio `SectionHeader` (título, breadcrumb, acción principal y botón de menú móvil) dentro de un `SectionTemplate`. `AppShell` y `Sidebar` no conocen la sesión: `Sidebar` acepta un slot `footer?: ReactNode` y `AppShell` lo reenvía como `sidebarFooter`, así que ambos siguen siendo componentes puros — sus stories no necesitan un `QueryClientProvider` — y quien conoce la sesión (`SidebarFooter`, feature `auth`) se inyecta solo desde `app/(dashboard)/layout.tsx`. Ese layout, y no `app/layout.tsx`, es quien aplica `AppShell`: las rutas del dashboard viven en el route group `app/(dashboard)/` para que `/login` (fuera del grupo) no herede el sidebar; los route groups no cambian las URLs. La ruta `/` ("Resumen") muestra tarjetas de métrica y accesos rápidos a las demás secciones (feature `overview`, que reutiliza los hooks de React Query existentes de `accounts`/`categories` sin nuevas llamadas HTTP). Las rutas `/cuentas`, `/categorias`, `/movimientos` y `/presupuestos` alojan los flujos completos: una tabla (`AccountsTable`/`CategoriesTable`/`TransactionsTable`/`BudgetsTable`) lista los registros con columna de estado (Activo/Inactivo) y acciones de editar y desactivar/reactivar por fila, y el botón principal del header ("Nueva cuenta"/"Nueva categoría"/"Nuevo movimiento"/"Nuevo presupuesto") abre un `Modal` con el formulario de alta; el mismo formulario se reutiliza en modo edición. `/configuracion` es la única página que sigue siendo un `EmptyState` ("próximamente"), sin fetching, para la sección que aún no tiene backend. `LoginForm` (feature `auth`) sigue el mismo patrón presentacional que los demás formularios, pero se muestra dentro de un `Panel` centrado en `/login`, no en un `Modal`. `packages/ui` aporta los tokens, atoms y molecules sin dominio.
...
- "Eliminar" nunca es un borrado físico: activa o desactiva el registro (ver [ADR-0002](../../docs/adr/0002-soft-delete-simetrico-cuentas-categorias.md) para cuentas/categorías, [ADR-0003](../../docs/adr/0003-desactivacion-reversible-movimientos.md) para movimientos y [ADR-0004](../../docs/adr/0004-presupuestos-mensuales-por-categoria.md) para presupuestos), es reversible y no requiere un modal de confirmación aparte.
- `proxy.ts` (reemplaza a `middleware.ts`, renombrado y deprecado en Next.js 16) verifica la cookie de sesión antes de cualquier ruta salvo `/login`, `/api`, `_next` y archivos estáticos, y redirige a `/login?from=` sin renderizar el dashboard (ver [ADR-0006](../../docs/adr/0006-autenticacion-sesion-cookie-firmada.md)).
...
- `TransactionsFilters` es un organism controlado (valores y `onChange` por props, sin estado de formulario propio) que administra cuenta, categoría, tipo, rango de fechas y estado; `TransactionsDashboard` traduce esos valores al query de la API. `AccountsTable` muestra el saldo real de cada cuenta (`GET /api/v1/transactions/balances`), no el saldo de apertura estático.
- `BudgetForm` reutiliza el mismo patrón de filtrado de `TransactionForm` para su selector de categoría (solo `expense`, activas o la ya asignada al editar) y agrega un campo `<input type="month">` que coincide exactamente con el `"YYYY-MM"` del contrato, sin conversión adicional. `BudgetsDashboard` mantiene el mes seleccionado como estado local (por defecto el mes actual) y `BudgetsTable` resalta en rojo la columna "Restante" cuando el gasto supera el límite.
